...
func SeedDatabase(dsn string) error {
	...
	log.Println("Setting default passwords for seeded customers...")
	_, err = db.Exec(`
		UPDATE customers 
	...
	`)
	if err != nil {
		log.Printf("Warning: failed to set default passwords: %v", err)
	}

	_, err = db.Exec(`ALTER TABLE customers ALTER COLUMN password_hash SET NOT NULL`)
	if err != nil {
		log.Printf("Warning: failed to restore password_hash NOT NULL: %v", err)
	}

	_, err = db.Exec(`UPDATE customers SET role = 'customer' WHERE role IS NULL OR role = ''`)
	if err != nil {
		log.Printf("Warning: failed to set default role: %v", err)
	}
