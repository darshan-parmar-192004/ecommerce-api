...
import { useNuxt, resolveFiles, resolvePath, logger, defineNuxtModule, addPlugin, addTypeTemplate, addTemplate, addComponent, updateTemplates, useNitro, addVitePlugin, addWebpackPlugin, addBuildPlugin, findPath, addImportsSources, tryResolveModule, isIgnored, tryUseNuxt, resolveAlias, addPluginTemplate, normalizeModuleTranspilePath, resolveNuxtModule, resolveIgnorePatterns, createResolver, nuxtCtx, addServerPlugin, installModule, addRouteMiddleware, loadNuxtConfig, normalizeTemplate, compileTemplate, normalizePlugin, templateUtils } from '@nuxt/kit';
...
async function initNuxt(nuxt) {
  ...
  const paths = Object.fromEntries(await Promise.all(coreTypePackages.map(async (pkg) => {
    if (dependencies.has(pkg) && !(pkg in nightlies)) {
      return [];
    }
    if (pkg in nightlies) {
      const nightly = nightlies[pkg];
      const path2 = await resolvePath$1(nightly, { url: nuxt.options.modulesDir }).then((r) => resolvePackageJSON(r)).catch(() => null);
      if (path2) {
        return [[pkg, [dirname(path2)]], [nightly, [dirname(path2)]]];
      }
    }
    const path = await resolvePath$1(pkg, { url: nuxt.options.modulesDir }).then((r) => resolvePackageJSON(r)).catch(() => null);
    if (path) {
...
async function loadNuxt(opts) {
  ...
  options.runtimeConfig = options.nitro.runtimeConfig;
