  ...
  <div class="admin-dashboard-page">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white dark:bg-brand-800 p-6 rounded-xl border border-gray-200 dark:border-brand-700">
        ...
        <div v-else class="space-y-3">
          <div v-for="(item, index) in lifetimeValues" :key="item.customer_id" class="flex items-center justify-between p-3 bg-gray-50 dark:bg-brand-700/50 rounded-lg">
            <div class="flex items-center gap-3">
              <span class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ item.customer_id }}</span>
            </div>
            <div class="text-right">
              <p class="text-sm font-bold text-gray-900 dark:text-gray-100">₹{{ item.lifetime_value?.toFixed(2) }}</p>
              <p class="text-xs text-gray-500 dark:text-gray-400">{{ item.total_orders || 0 }} orders</p>
            </div>
          </div>
        </div>
      </div>
