...
<template>
  <aside
    class="glass-card rounded-2xl p-6 mb-8 transition-all duration-500 ease-out hover:shadow-3xl hover:bg-white/25 dark:hover:bg-brand-800/25 hover:backdrop-blur-3xl"
  ...
  >
    <form @submit.prevent="$emit('apply')" class="space-y-5">
      ...
      <!-- Category Dropdown -->
      <div class="relative">
        <label for="category-select" class="block text-sm font-semibold text-gray-800 dark:text-gray-300 mb-2">Category</label>
        <div class="relative">
          <select
            id="category-select"
            :value="categoryValue"
      ...
      <div class="grid grid-cols-2 gap-4">
        <div class="relative">
          <label for="min-price" class="block text-sm font-semibold text-gray-800 dark:text-gray-300 mb-2">Min Price</label>
          <div class="relative">
            ...
            <input
              ...
              min="0"
              step="0.01"
              class="w-full pl-8 pr-4 py-3.5 rounded-xl border-2 border-white/30 dark:border-brand-600/30 bg-white/15 dark:bg-brand-800/15 backdrop-blur-xl text-gray-900 dark:text-gray-100 placeholder:text-gray-500/70 dark:placeholder:text-gray-400/70 transition-all duration-300 hover:bg-white/20 dark:hover:bg-brand-800/20 hover:border-white/40 dark:hover:border-brand-600/40 focus:bg-white/25 dark:focus:bg-brand-800/25 focus:border-gray-400/60 dark:focus:border-brand-500/60 focus:ring-4 focus:ring-gray-900/20 dark:focus:ring-brand-700/20 focus:outline-none"
            />
          </div>
        </div>
        <div class="relative">
          <label for="max-price" class="block text-sm font-semibold text-gray-800 dark:text-gray-300 mb-2">Max Price</label>
          <div class="relative">
            <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 font-medium">₹</span>
            <input
              id="max-price"
      ...
      <div class="flex flex-col sm:flex-row gap-3 pt-3">
        ...
        <button
          type="button"
          @click="$emit('clear')"
          class="px-6 py-3.5 border-2 border-gray-300/60 dark:border-brand-600/60 text-gray-700 dark:text-gray-300 font-semibold rounded-xl hover:bg-white/30 dark:hover:bg-brand-800/30 hover:border-white/50 dark:hover:border-brand-600/50 active:scale-[0.98] transition-all duration-300 focus:ring-4 focus:ring-gray-300/50 dark:focus:ring-brand-600/50 focus:outline-none backdrop-blur-sm"
        >
          Clear All
        </button>
      </div>
