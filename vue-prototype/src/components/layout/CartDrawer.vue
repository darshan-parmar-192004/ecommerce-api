...
const { items, isDrawerOpen, cartTotal, toggleDrawer } = useCart()
</script>

<template>
  <Transition name="drawer">
    <div v-if="isDrawerOpen" class="fixed inset-0 z-50 overflow-hidden">
      <div class="absolute inset-0 bg-black/50" @click="toggleDrawer()" />
      <div class="absolute right-0 top-0 h-full w-full sm:w-full sm:max-w-md md:max-w-lg bg-white dark:bg-brand-800 shadow-xl flex flex-col" role="dialog" aria-modal="true" aria-label="Shopping Cart">
        <div class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-brand-700">
          ...
          <button @click="toggleDrawer()" class="p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" aria-label="Close cart">
            <X class="w-5 h-5" />
          </button>
        </div>

        <div class="flex-1 overflow-y-auto p-4">
          <TransitionGroup v-if="items.length > 0" name="cart-item" tag="div" class="space-y-4">
            <CartItem
              v-for="item in items"
              :key="item.id"
              :item="item"
        ...
        <div v-if="items.length > 0" class="border-t border-gray-200 dark:border-brand-700 p-4 space-y-4">
