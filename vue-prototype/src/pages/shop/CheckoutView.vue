...
const { profile, createOrder, fetchProfile } = useUser()
...
onMounted(async () => {
  await fetchProfile()
  if (profile.value) {
    shippingInfo.address = profile.value.address || ''
    shippingInfo.city = profile.value.city || ''
    shippingInfo.state = profile.value.state || ''
    shippingInfo.zip = profile.value.zip || ''
    shippingInfo.country = profile.value.country || ''
  }
})
...
const placeOrder = async () => {
  ...
  try {
    ...
    if (!profile.value?.customerId) {
      showError(new Error('User profile not loaded. Please try again.'))
      return
    }
    ...
    orderId.value = result.orderId || 'N/A'
    orderPlaced.value = true
    clearCart()
    showSuccess('Order placed successfully!')
  } catch (err) {
    showError(err, 'Failed to place order. Please try again.')
  } finally {
    loading.value = false
  }
}
    ...
    <div v-else class="space-y-8">
      ...
      <div v-else-if="step === 2" class="space-y-6">
        ...
        <div class="bg-gray-50 dark:bg-brand-800 p-4 rounded-lg space-y-3">
          <div v-for="item in items" :key="item.id" class="flex justify-between text-sm text-gray-700 dark:text-gray-300">
        ...
        <div class="flex gap-4">
          ...
          <button @click="placeOrder" :disabled="loading || items.length === 0" class="btn-primary flex-1">
